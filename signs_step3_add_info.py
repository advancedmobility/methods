...
for unitdir in all_units:
    ...
    if os.path.exists(sign_locator_file):
        ...
        leg_time, conspi_ratio, glan_leg, und_max, sign_info, sign_side = [], [], [], [], [], []
        ...
        for i in range(len(sign_data['features'])):
            ...
            score_leg.append(sign_data['features'][i]['properties']['legibility_time_score'])
            conspi_ratio.append(sign_data['features'][i]['properties']['conspicuity'])
            score_csp.append(sign_data['features'][i]['properties']['conspicuity_score'])
            glan_leg.append(sign_data['features'][i]['properties']['glance_legibility'])
            score_gla.append(sign_data['features'][i]['properties']['glance_legibility_score'])
            und_max.append(sign_data['features'][i]['properties']['understandability'])
            score_und.append(sign_data['features'][i]['properties']['understandability_score'])
        ...
        if len(sign_score) > 0:
            ...
            for j in range(len(sign_lrs)):
                ...
                sign_prop = {
                    ...
                    "legibility_time_score": score_leg[j],
                    "conspicuity": conspi_ratio[j],
                    "conspicuity_score": score_csp[j],
                    "glance_legibility": glan_leg[j],
                    "glance_legibility_score": score_gla[j],
                    "understandability": und_max[j],
                    "understandability_score": score_und[j],
                    "side":sign_side[j],
                    "info": sign_info[j]
                }
                sign_dict = {
